...
import com.oxycab.provider.base.BaseFragment;
import com.oxycab.provider.common.SharedHelper;
import com.oxycab.provider.data.network.model.Request_;
import com.oxycab.provider.data.network.model.VehicleTags;
...
import static com.oxycab.provider.base.BaseActivity.time_to_left;
...
public class IncomingRequestFragment extends BaseFragment implements IncomingRequestIView, DirectionCallback {
    ...
    TextView lblCarType;
    @BindView(R.id.tvSelectedService)
    TextView tvSelectedService;
    @BindView(R.id.tvSelectedServiceTitle)
    TextView tvSelectedServiceTitle;
    ...
    void init() {
        ...
        if (data != null) {
            ...
            StringBuilder vehicleTags = new StringBuilder();
            if (!data.getVehicleTags().isEmpty()) {
                for (VehicleTags item : data.getVehicleTags()) {
                    vehicleTags.append(item.getVehicleTag().getName()).append("\n");
                }
                tvSelectedService.setText(vehicleTags);
            } else {
                tvSelectedServiceTitle.setVisibility(View.GONE);
            }
